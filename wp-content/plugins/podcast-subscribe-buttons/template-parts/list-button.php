		...
		if( !empty($secondline_psb_subscribe_entries) ) {
			...
			foreach ( $secondline_psb_subscribe_entries as $key => $entry ) {
				...
				if(($secondline_psb_link != '') && ($secondline_psb_platform_slt != '') && ($secondline_psb_platform_slt != 'custom') ) {
					echo '<li class="secondline-psb-subscribe-'.esc_attr($secondline_psb_platform_slt).'"><a onMouseOver="this.style.color=`'. esc_attr( $atts[SECONDLINE_PSB_PREFIX.'text_color_hover'] ) .'`; this.style.backgroundColor=`'. esc_attr($atts[SECONDLINE_PSB_PREFIX.'background_color_hover']) .'`" onMouseOut="this.style.color=`'. esc_attr($atts[SECONDLINE_PSB_PREFIX.'text_color']) .'`; this.style.backgroundColor=`'. esc_attr($atts[SECONDLINE_PSB_PREFIX.'background_color']) .'`" style="color:'. esc_attr($atts[SECONDLINE_PSB_PREFIX.'text_color']) .'; background-color:'. esc_attr($atts[SECONDLINE_PSB_PREFIX.'background_color']) .'" class="button podcast-subscribe-button" href="' . esc_url($secondline_psb_link) . '" target="_blank"><img class="secondline-psb-subscribe-img" src="'. SECONDLINE_PSB_SUBSCRIBE_ELEMENTS_URL .'assets/img/icons/' . esc_attr($secondline_psb_platform_slt) . secondline_psb_icon_extension( $secondline_psb_platform_slt ) .'" alt="' . esc_attr($secondline_psb_platform_text) . '" />' . esc_html($secondline_psb_platform_text) . '</a></li>';
