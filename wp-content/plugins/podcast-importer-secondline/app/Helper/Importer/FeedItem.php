...
class FeedItem {
  ...
  public function __construct( PIS_Helper_Importer $importer, $feed_item ) {
    ...
    $this->current_post_id = intval($wpdb->get_var('SELECT post_id FROM ' . $wpdb->postmeta . ' WHERE ( meta_key = "secondline_imported_guid" AND meta_value LIKE "%' . $wpdb->esc_like( $this->_guid ) . '%")' ) );
  ...
  private function _set_post_information() {
    ...
    // This one can be uncommented if wanted.
    // do_action( 'secondline_after_post_scheduled_import', $this->current_post_id, $this->importer->feed_link, $this->feed_item );
  }
